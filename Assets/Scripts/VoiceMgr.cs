...
public class VoiceMgr : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        keywordCollection.Add("menu", StatusMenu);

        keywordCollection.Add("capture", CaptureMenu);
        keywordCollection.Add("snap", SnapMenu);
    ...
    //"status" 키워드에 반응할 메소드
    private void StatusMenu(PhraseRecognizedEventArgs args)
    {
        if (ReticleMgr.focusedObj != null)
        {
            statusUI.GetComponent<Button>().onClick.Invoke();
        }
    }
    ...
    //"sajin" 키워드에 반응할 메소드
    private void SnapMenu(PhraseRecognizedEventArgs args)
    {
        if (ReticleMgr.focusedObj != null)
        {
            snapScreen.GetComponent<Button>().onClick.Invoke();
        }
    }
